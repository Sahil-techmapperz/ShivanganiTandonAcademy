<section class="relative h-[500px] w-full overflow-hidden flex items-center justify-center text-center">
    <div
        class="absolute inset-0 z-0 bg-fixed bg-center bg-cover bg-no-repeat filter grayscale"
        style="background-image: url('<?= base_url('public/images/EA/Who_is_an_IRS/d34c93b23614324ecdcdd2ae34653a8e670b313b.jpg') ?>');">
    </div>

    <div class="absolute inset-0 z-10 bg-black/50"></div>

    <div class="relative z-20 max-w-7xl">
        <p class="text-white font-poppins font-bold text-[28px] sm:text-[32px] md:text-[36px] lg:text-[48px] leading-[36px] sm:leading-[44px] md:leading-[56px] lg:leading-[78px] text-center capitalize mb-6 sm:mb-8">
            Who Is An IRS Enrolled Agent?
        </p>

        <p class="text-gray-200 font-poppins font-normal text-sm sm:text-[16px] md:text-[18px] leading-[22px] sm:leading-[25px] md:leading-[28.8px] text-center capitalize mb-6 sm:mb-8 md:mb-10 mx-auto">
            EAs were established by the Department of Treasury/IRS on July 7, 1887. EAs must complete a rigorous background check and a
            difficult 3-part exam (IRS Special Enrollment Examination) that tests your ability to prepare federal income tax returns and the
            ability to represent taxpayers before proceedings of Internal Revenue Service (IRS).
        </p>

        <button class="group inline-flex items-center bg-[#FFC224] hover:bg-[#FFC224] text-slate-900 font-bold 
                                text-sm sm:text-base md:text-lg 
                                py-1.5 sm:py-2 md:py-2 
                                px-3 sm:px-4 md:px-6 
                                rounded-full flex items-center justify-center 
                                transition-all duration-300 ease-in-out 
                                shadow-[4px_4px_0_0_#3D3D3D] 
                                hover:-translate-y-1 hover:shadow-[6px_8px_0px_0px_#3D3D3D] 
                                active:translate-y-1 active:shadow-[2px_3px_0px_0px_#050071]">
            Know More <span class="ml-2 text-sm sm:text-base md:text-lg">→</span>
        </button>
    </div>
</section>