    ...
    <div class="app-content-header py-5">
        <div class="container-fluid">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h1 class="display-5 fw-bold text-dark mb-1">Support & Help Requests</h1>
                    <p class="text-muted lead mb-0">Review and manage incoming student help inquiries.</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table id="helpTable" class="table table-hover align-middle mb-0 custom-table">
                    ...
                    <div id="loadingState" class="text-center py-5">
                        ...
                        <p class="mt-2 text-muted">Loading support requests...</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header bg-primary text-white border-0 py-3">
                <h5 class="modal-title fw-bold">Support Details</h5>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow rounded-4">
            <div class="modal-body p-4 text-center">
                ...
                <p class="text-muted mb-4">Are you sure you want to delete this support request? This action cannot be undone.</p>
...
<script>
    document.addEventListener("DOMContentLoaded", loadhelpRequest);
    let helpData = [];

    function loadhelpRequest() {
        const tbody = document.querySelector("#helpTable tbody");
        const loading = document.getElementById("loadingState");
        loading.style.display = "block";
        tbody.innerHTML = "";

        fetch("<?= base_url('api/help_request') ?>")
            .then(res => res.json())
            .then(response => {
                ...
                if (helpData.length > 0) {
                    ...
                    tbody.innerHTML = `<tr><td colspan="4" class="text-center py-5 text-muted">No help requests found</td></tr>`;
                }
            })
            .catch(err => {
                loading.style.display = "none";
    ...
    function viewDetails(index) {
        ...
        modalBody.innerHTML = `
            ...
            <div class="detail-label">Request Message</div>
            <div class="bg-light p-3 rounded-3" style="white-space: pre-line;">${item.message || 'No message provided'}</div>
        `;
    ...
    function confirmDelete() {
        ...
        fetch(`<?= base_url('api/help_request') ?>/${id}`, { method: 'DELETE' })
            .then(res => res.json())
            .then(resp => {
