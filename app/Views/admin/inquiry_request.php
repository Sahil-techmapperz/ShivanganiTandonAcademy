    ...
    <div class="app-content-header py-5">
        <div class="container-fluid">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h1 class="display-5 fw-bold text-dark mb-1">Academy Inquiries</h1>
                    <p class="text-muted lead mb-0">Track and manage course interest and enrollment inquiries.</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table id="inquiryTable" class="table table-hover align-middle mb-0 custom-table">
                            <thead class="bg-light bg-opacity-50">
                                <tr>
                                    <th class="ps-4 py-3 text-muted small fw-bold border-0">STUDENT NAME</th>
                                    <th class="py-3 text-muted small fw-bold border-0">MOBILE</th>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow rounded-4">
            <div class="modal-body p-4 text-center">
                ...
                <p class="text-muted mb-4">Are you sure you want to delete this inquiry? This action cannot be undone.</p>
...
<script>
    ...
    function loaddemorequest() {
        ...
        fetch("<?= base_url('api/inquiry_request') ?>")
            .then(res => res.json())
            .then(response => {
                ...
                if (inquiryData.length > 0) {
                    inquiryData.forEach((item, index) => {
                        ...
                        const row = `
                            <tr>
                                ...
                                <td><div class="fw-medium">${item.mobile || '-'}</div></td>
                                <td><span class="badge bg-primary-subtle text-primary rounded-pill px-3">${item.current_status || '-'}</span></td>
                    ...
                    tbody.innerHTML = `<tr><td colspan="5" class="text-center py-5 text-muted">No inquiries found</td></tr>`;
                }
            })
            .catch(err => {
                loading.style.display = "none";
        ...
        modalBody.innerHTML = `
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <div class="detail-value text-primary">${item.current_status || '-'}</div>
                </div>
                <div class="col-md-6">
                    <div class="detail-label">Mobile Number</div>
                    <div class="detail-value">${item.mobile || '-'}</div>
                </div>
                <div class="col-md-6">
                    <div class="detail-label">Area of Interest</div>
                    <div class="detail-value">${item.area_of_interest || '-'}</div>
                </div>
                <div class="col-12">
    ...
    function confirmDelete() {
        ...
        fetch(`<?= base_url('api/inquiry_request') ?>/${id}`, { method: 'DELETE' })
            .then(res => res.json())
            .then(resp => {
